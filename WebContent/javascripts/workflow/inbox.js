...
(rxInbox = {
							...
							if(task) {   
								 ...
								 if (stateId !== '') {
									 ...
									 success:function (oResponse) {
										 ...
										 rxInbox.renderHistory(historyData);
										 document.getElementById('loadImg').style.display = 'none';
									 }, 
		...
		'initInbox' : function() {
			...
			cal.selectEvent.subscribe(rxInbox.setSelectedDate, cal, true);
			rxInbox.refresh();
			//window.setInterval(rxInbox.autoPoll,900000);
		},
		'forDate' : 'fromDate',
		'setSelectedDate' : function (type,args,obj) {
		  ...
		  var day = datedata[2];  
		  document.getElementById(rxInbox.forDate).value = day+'/'+month+'/'+year;
		  obj.hide();
		},
...
YAHOO.util.Event.onDOMReady(rxInbox.initInbox);